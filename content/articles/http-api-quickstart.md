...
date = "2025-08-28"
description = "Изучаем основы клиент-серверного взаимодействия и работы с API на практике."
...
В серии будет три статьи, в которых разберём следующее:

1. → Увидим, как устроено клиент-серверное взаимодействие;
1. [Понаблюдаем за тем, как браузер получает и отображает данные, а также изучим структуру веб-страниц](@/articles/using-devtools.md);
1. [Развернём собственный локальный сервер, чтобы убедиться, что сервер — это просто программа](@/articles/web-server.md).

В первой статье мы познакомимся с основой основ — запросом к HTTP API и ответом сервера. HTTP до сих пор остаётся самым популярным протоколом для клиент-серверного взаимодействия, и понимание его работы поможет разобраться по аналогии с любым другим протоколом.
...
Перед тем как вы приступите к теории, запомните: какие-то части могут показаться слишком сложными и их, скорее всего, можно пропустить, чтобы вернуться к ним в будущем. Не стесняйтесь переходить к [практике](#practice), если теория не даётся.
...
Принципы REST достаточно просты и их немного, я выделю наиболее важные на первом знакомстве:
...
Эта статья предполагает работу с `curl` в терминале с поддержкой оболочки **Bash**.

На **Windows** я рекомендую использовать **Git Bash**, который устанавливается вместе с [Git for Windows](https://git-scm.com/downloads/win). Если у вас есть другой предпочитаемый терминал, который поддерживает Bash, можно использовать его. Работа в PowerShell технически возможна, но это не рекомендуемый вариант.
...
Для проверки статуса используется *флаг* `-I`, который запрашивает только заголовки HTTP-ответа и не скачивает тело ответа:
...
Ожидаемый ответ с содержимым поста, не несущим особой смысловой нагрузки:
...
* У постов есть комментарии, доступные по следующему пути: `/posts/<номер>/comments`.
...
В ответе будет множество альбомов этого пользователя. `id` в данном случае — это идентификатор альбома:
...
Получим массив фотографий, начинающийся так:
...
POST-запросы в `curl` требуют указания большего количества параметров:
