...
flowchart TD
    ...
    N --> N_Loc[Локально:<br/>Qwen 3.6 35B, GPT-OSS 120b]
    N --> N_API[Китайские модели в API:<br/>GLM 5.1, DeepSeek 4, Kimi k2.6]

    H --> H_Loc[Локально:<br/>GLM 4.7, Kimi k2.6, Minimax 2.5, DeepSeek 3.2]
...
**Если данные можно отправлять наружу — начинайте с фронтирных моделей через API.** Оцените выгоду на реальных задачах, а затем ищите более дешёвые альтернативы для оптимизации стоимости.
...
1. **Трансформировать речь в текст с разбивкой по спикерам.** Это самая простая задача, которую могут решить модели dhjlt [Whisper](https://github.com/openai/whisper) или десятки других качественных speech-to-text моделей.
...
- **Эффективное контекстное окно** — 128K от производителя может означать 32K–64K до деградации. Проверить можно на релеватных бенчмарках и своих собственных тестах. Проверяется по методу Needle In A Haystack, типичный бенчмарк — **LongBench**.
